...
**Substrate person dedupe (`identity_fingerprint`):** accent-folded name (`person_match_key`) plus **affiliation** only — **title/position is excluded** because it varies by article and is not part of tier-1 canonical match. Affiliation stays in the fingerprint so two namesakes with different teams remain separate substrate rows until canonical policy resolves them.
...
|---------|----------------|------------------|-------|
| **LLM canonical adjudication** | Ambiguous recall under `ai_assisted` | Person: [`worker/…/person/adjudication.py`](../apps/worker/src/worker/substrate/entities/person/adjudication.py). Location: [`worker/…/canonical/adjudication.py`](../apps/worker/src/worker/substrate/canonical/adjudication.py) after policy `DEFER` with `ambiguous_canonical_match` (includes **gate-demoted** recall per `entities/location/policy.py`) | Link only if model confidence ≥ `ADJUDICATION_LINK_MIN_CONFIDENCE` (0.9); declined link → `MATERIALIZE_NEW` when materialize-after-recall gates allow (person: blocked by PersonExtract `flag_review` / `auto_defer`). **Person athletes/public figures:** adjudication prompt treats team/position changes as non-discriminators; declined link **defers to human review** instead of materializing when `public_figure` or sports `person_type`. Payload includes article mention snippet and candidate aliases. Adjudication LLM calls run in parallel per domain during DBOutput (default **8** concurrent via `CANONICAL_ADJUDICATION_MAX_CONCURRENT`); catalog writes remain serial. |
| **Extract review routing** | Extract emits review codes (waive vs flag queue) | [`entities/person/review.py`](../packages/backfield-entities/src/backfield_entities/entities/person/review.py) | PersonExtract: `child` / `animal` / `not_a_person` → auto-waived on ingest when `auto_apply_canonicalization`, but candidate AI review recommends **defer**. `pseudonym` and descriptive “role in place” names (e.g. `TRUTH-TELLER IN ARKANSAS`) → defer recommendation only. `stage_name_or_alias` and surname-inferred rows run recall + LLM adjudication like other names (link/create recommendations) but never auto-materialize on ingest. `first_name_only` (without inferred surname) → defer recommendation only. Deterministic `not_a_person` heuristics also catch mis-tagged schools, laws, and media outlets already in the queue. |
...
## Automatic entity connections

Manual Stylebook connections require at least one of free-form **`description`** (human-readable relationship sentence) or **`nature`** (normalized slug). Automatic connection inference (Backfield Output, post-canonicalization) is **description-first**: every auto-created edge includes a concise **`description`**, quote-backed evidence, and confidence ≥ 0.9; **`nature`** is set only when a controlled taxonomy slug clearly fits, otherwise **`null`**. Families include **`person → organization`**, **`organization → location`**, **`person → location`**, **`person → person`**, and **`organization → organization`**, with machine-slug natures and location-granularity gates where applicable. **Person→organization** may also use deterministic affiliation hints (`affiliation_links`); **organization→location** uses **same-site name hints** (`same_site_hints`) when an org's primary name matches a co-mentioned place's primary name (e.g. school org and school place)—hints enrich the family LLM pass and may trigger a focused pair review; edges are still LLM-confirmed with quotes. Auto-created rows store optional **`evidence_json`** (including **`match_basis`** when applicable) on **`stylebook_connections`**; exact edges are unique per **`(project_id, from, to, coalesce(nature,''), coalesce(description,''))`**. See **`docs/DATABASE.md`** revisions **`040_sb_conn_evidence`** and **`061_sb_conn_description`**.

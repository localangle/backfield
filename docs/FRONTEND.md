...
- **Stylebook home tabs:** the catalog dashboard (`**/stylebook/<slug>/**`) and cleanup routes share a tab strip (**Entities** | **Cleanup**) via [`StylebookHomeTabs`](../apps/stylebook-ui/src/components/StylebookHomeTabs.tsx). **Cleanup** is stylebook-scoped data-quality triage only—no automatic fixes. v1 ships two location checks: **Possible duplicate locations** (`**/cleanup/duplicate-locations**`, clusters exact same-name rows and near-duplicates that share a primary name with high full-label similarity; Postgres uses a head-label index for blocking rather than a full-table trigram self-join) and **Locations missing geography** (`**/cleanup/missing-geometry-locations**`, flat list via `**…/cleanup/checks/missing-geometry-locations**`). The hub (`**/cleanup**`) lists checks with counts from `**GET …/cleanup/checks**`. Each flagged row deep-links into the existing location canonical detail editor.

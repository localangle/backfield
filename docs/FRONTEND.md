...
- **Stylebook home tabs:** the catalog dashboard (`**/stylebook/<slug>/**`) and cleanup routes share a tab strip (**Entities** | **Checks**) via [`StylebookHomeTabs`](../apps/stylebook-ui/src/components/StylebookHomeTabs.tsx). The **Checks** tab is stylebook-scoped data-quality triage. The hub (`**/cleanup**`) lists checks in a table with location / people / organization icons. On load it calls `**GET …/cleanup/checks**` for durable per-check **status** (`never_run`, `queued`, `running`, `succeeded`, `failed`), **issue counts** (from the latest successful run, after dismissals), and **last run** timestamps from the backend (`**completed_at**` / `**ran_at**`). Checks **do not auto-run** on page open — each row has **Run** (`**POST …/cleanup/checks/{check_id}/runs**`), polls `**GET …/cleanup/checks/{check_id}/runs/latest**` until the run finishes, and shows a per-row spinner while **queued** or **running**. A colored staleness dot with short text under **Last run** uses the same thresholds as before (green when run within 2 days, yellow at 2–4 days, red at 5+ days or never run). Detail pages paginate cached candidates from the latest successful run; when no run exists yet they show an empty state directing editors back to the hub **Run** action. Copy explains that checks surface common curation issues on canonical objects. Location checks: **Potential duplicate locations** (cluster view with inline **drag-to-merge**, **delete empty**, and **Keep separate** when the caller has stylebook edit access; merge relinks linked places via `**POST …/cleanup/canonical-locations/{source_id}/merge-into**`, then deletes the source; **Keep separate** persists pair dismissals via `**POST …/cleanup/dismissals**` so only new duplicate pairs resurface later) and **Potential missing or incorrect geographies** (flat list with **Mark reviewed** per row, same dismiss endpoint with a single canonical id). People and organizations each have a **Potential duplicate …** cluster check with the same inline merge/delete/**Keep separate** UX (`**…/cleanup/canonical-people/**` and `**…/cleanup/canonical-organizations/**`), plus a **Potential mismatched …** list check (`**GET …/cleanup/checks/mismatched-people**` / `**mismatched-organizations**`) that flags linked story records whose names look unlike the catalog label (example offending names inline; **Mark reviewed** dismisses per canonical). Duplicate cluster checks also offer **Review with AI** (model picker from the org’s active text/json pool; background job via `**POST …/cleanup/ai-review**`; poll `**GET …/cleanup/ai-review/{id}**`; accept/reject proposals with `**POST …/cleanup/ai-review/proposals/{id}/accept|reject**` — accept applies the same merge/dismiss operations as manual cleanup). Duplicate cluster detail pages list **exact same-name** groups first, then near-duplicates, and include a **`q`** name filter (same query key as canonical lists). Each flagged row deep-links into the matching canonical detail editor.

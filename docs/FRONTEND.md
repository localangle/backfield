...
- **Person candidates review queue:** `**/stylebook/<slug>/people/candidates`** mirrors the location queue (`**GET /v1/people/candidates**`, flat pagination, open/deferred tabs). **Link to canonical** opens [`PersonCanonicalLinkModal`](../apps/stylebook-ui/src/components/PersonCanonicalLinkModal.tsx): `**GET /v1/people/candidates/{id}/suggested-canonicals**` plus catalog search `**GET /v1/canonical-people?q=…**`, with the search field **pre-filled** from the candidate display name; confirm via `**POST /v1/people/{substrate_person_id}/link-canonical**`. **UX parity with locations:** create-modal similar-canonical nudge, post-create “potential links” toast/dialog, and `canonical_review_lines` under rows—see [`ENTITY_TYPES.md`](ENTITY_TYPES.md) → **Candidate queue UX parity** and shared modules under `apps/stylebook-ui/src/lib/candidateQueue*.ts` and `apps/stylebook-ui/src/components/{CreateCanonicalLinkNudgeAlert,PotentialCandidateLinksDialog,CandidateReviewReasons}.tsx`. Implementation checklist for new types: [`ENTITY_TYPES.md`](ENTITY_TYPES.md) → **Per-type implementation patterns**.

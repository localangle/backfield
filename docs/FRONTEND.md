...
- **Processed item JSON tab:** when the API returns **`reviewed_output`** (saved place or story review), the tab defaults to **Reviewed**; **Original** shows immutable run **`output`**. A single **Download** button exports whichever version is selected. When there is no reviewed output, only original output is shown. When the flow wrote the item to S3 (an **S3 Output** payload with `s3_bucket` / `s3_key` in the run JSON), the header shows the saved S3 location plus last-sync status, and a **Sync to S3** button (batch items only) asks for confirmation, then calls `**POST /runs/{id}/items/{item_id}/s3-sync**` to overwrite the existing S3 file with the current output — including review changes (see `lib/review/content/s3OutputSync.ts`).

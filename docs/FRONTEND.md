...
- **Catalog scope (multiple Stylebooks per org):** when the org has more than one catalog, the URL also carries **`?stylebook=<stable-slug>`** (and the shell shows a **Catalog** control). The Stylebook UI’s shared fetch helper mirrors that to Stylebook API calls as `stylebook_slug` so list/detail/candidate/connection requests target the selected catalog; omitting it keeps the **workspace** catalog for the project. Agate **GET /projects** includes optional **`workspace_stylebook_slug`** so the UI can default the query when you pick a project.
...
- **Org admin — catalogs (Agate UI):** organization admins open **Manage catalogs** from the account menu (when the host passes `onManageCatalogs`), which routes to **`/admin/catalogs`**. The page calls Stylebook API org stylebook routes via `**src/lib/stylebook-org-api.ts**` (default base **`/api/stylebook`**, same session cookie). After create/rename/default/remove, the UI dispatches **`agate:workspaces-changed`** so the hub sidebar can refresh workspace/catalog context.

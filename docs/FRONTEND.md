...
- **Core API (login / session):** `POST /v1/auth/login`, `GET /v1/auth/me`, `POST /v1/auth/logout`, `POST /v1/auth/change-password`. `GET /v1/auth/me` returns `organization_name` (publication / tenant display name). `GET /v1/me/workspaces` lists workspaces and visible projects for the signed-in user (used on `/`, `/workspace/:workspaceSlug`, and in the sidebar). Organization admin routes under `/v1/organizations/{org_id}/…` (users, workspaces, workspace rename, workspace memberships for access control). Use `**VITE_AUTH_API_BASE`** (empty string for same-origin). Typed fetch helpers live in `**src/lib/core-api.ts**` (session cookie, `credentials: 'include'`).

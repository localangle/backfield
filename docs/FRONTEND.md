...
- **First-slice routes (supported):** `/` (dashboard), `/locations/candidates`, `/locations/canonical` (lists **`GET /v1/canonical-locations`**, ids are **canonical** ids), `/locations/canonical/:id` (detail uses **`GET /v1/canonical-locations/{id}`** and **`…/mentions`**), `/locations/create` (creates a substrate row **and** materializes a canonical; navigate using **`stylebook_location_canonical_id`** from **`POST /v1/locations`**), plus **stub** pages for import, other entity candidates, and agents so deep links do not 404.

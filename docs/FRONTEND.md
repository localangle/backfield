...
- **Stylebook home tabs:** the catalog dashboard (`**/stylebook/<slug>/**`) and cleanup routes share a tab strip (**Entities** | **Cleanup**) via [`StylebookHomeTabs`](../apps/stylebook-ui/src/components/StylebookHomeTabs.tsx). **Cleanup** is stylebook-scoped data-quality triage. Location checks: **Possible duplicate locations** (cluster view with inline **drag-to-merge** and **delete empty** when the caller has stylebook edit access; merge relinks linked places via `**POST …/cleanup/canonical-locations/{source_id}/merge-into**`, then deletes the source) and **Missing or potentially incorrect geographies** (flat list of catalog records with no stored geometry or linked places far from the catalog map location). People and organizations each have a **Possible duplicate …** cluster check with the same inline merge/delete UX (`**…/cleanup/canonical-people/**` and `**…/cleanup/canonical-organizations/**`). The hub lists all checks with counts from `**GET …/cleanup/checks**`. Each flagged row deep-links into the matching canonical detail editor.

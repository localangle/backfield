...
- **Catalog scope (multiple Stylebooks per org):** the URL carries **`?stylebook=<stable-slug>`** when a catalog is selected. The Stylebook app lists org catalogs in the **left sidebar**; choosing one updates the query. The shared fetch helper mirrors that to Stylebook API calls as `stylebook_slug` so list/detail/candidate/connection requests target the selected catalog; when the param is omitted (for example a single-catalog org), the UI still infers the workspace / default catalog the same way as before. Agate **GET /projects** includes optional **`workspace_stylebook_slug`** so the UI can default the query when you pick a project.
...
- **Org admin — stylebooks (Agate UI):** organization admins open **Manage stylebooks** from the account menu (when the host passes `onManageCatalogs`), which routes to **`/admin/catalogs`**. Stylebook UI passes the same callback for org admins (full-page navigation to Agate; see Stylebook **Auth** bullet above). The page calls Stylebook API org stylebook routes via `**src/lib/stylebook-org-api.ts**` (default base **`/api/stylebook`**, same session cookie). After create/rename/default/remove, the UI dispatches **`agate:workspaces-changed`** so the hub sidebar can refresh workspace and stylebook context.

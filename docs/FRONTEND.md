...
- **Scope key (first slice):** the UI keeps a **`project_slug`** in the query string (`?project=…`) as the primary navigation scope. `**stylebook-api**` resolves the slug to `backfield_project.id`. `**substrate_location**` rows are project-scoped place instances; the Stylebook **catalog** is `**stylebook_location_canonical`** (one row per editorial canonical), linked via `**substrate_location.stylebook_location_canonical_id**`.
...
- **Stylebook home tabs:** the catalog dashboard (`**/stylebook/<slug>/**`) and cleanup routes share a tab strip (**Entities** | **Review**) via [`StylebookHomeTabs`](../apps/stylebook-ui/src/components/StylebookHomeTabs.tsx). The **Review** tab is stylebook-scoped data-quality triage. The hub (`**/cleanup**`) lists reviews in a table with location / people / organization icons. On load it calls `**GET …/cleanup/checks**` for durable per-review **status** (`never_run`, `queued`, `running`, `succeeded`, `failed`), **issue counts** (from the latest successful run, after dismissals), and **last run** timestamps from the backend (`**completed_at**` / `**ran_at**`). Reviews **do not auto-run** on page open — each row has **Run** (`**POST …/cleanup/checks/{check_id}/runs**`), polls `**GET …/cleanup/checks/{check_id}/runs/latest**` until the run finishes, and shows a per-row spinner while **queued** or **running**. A colored staleness dot with short text under **Last run** uses the same thresholds as before (green when run within 2 days, yellow at 2–4 days, red at 5+ days or never run). Detail pages paginate cached candidates from the latest successful run; when no run exists yet they show an empty state directing editors back to the hub **Run** action. Copy explains that reviews surface common curation issues on canonical records. Location reviews: **Potential duplicate locations** (cluster view with inline **drag-to-merge**, **delete empty**, and **Keep separate** when the caller has stylebook edit access; merge relinks linked places via `**POST …/cleanup/canonical-locations/{source_id}/merge-into**`, then deletes the source; **Keep separate** persists pair dismissals via `**POST …/cleanup/dismissals**` so only new duplicate pairs resurface later) and **Potential missing or incorrect geographies** (flat list with **Mark reviewed** per row, same dismiss endpoint with a single canonical id). People and organizations each have a **Potential duplicate …** cluster review with the same inline merge/delete/**Keep separate** UX (`**…/cleanup/canonical-people/**` and `**…/cleanup/canonical-organizations/**`), plus a **Potential mismatched …** list review (`**GET …/cleanup/checks/mismatched-people**` / `**mismatched-organizations**`) that flags linked story records whose names look unlike the catalog label (example offending names inline; **Mark reviewed** dismisses per canonical). Duplicate cluster reviews also offer **Review with AI** (model picker from the org’s active text/json pool; background job via `**POST …/cleanup/ai-review**`; poll `**GET …/cleanup/ai-review/{id}**`; accept/reject proposals with `**POST …/cleanup/ai-review/proposals/{id}/accept|reject**` — accept applies the same merge/dismiss operations as manual cleanup). Duplicate cluster detail pages list **exact same-name** groups first, then near-duplicates, and include a **`q`** name filter (same query key as canonical lists). Each flagged row deep-links into the matching canonical detail editor.
- **First-slice routes (supported):** `**/`** redirects to `**/stylebook/<slug>/**` (dashboard); nested segments include `**cleanup**` and `**cleanup/:checkId**` (see **Stylebook home tabs** above), `**locations/candidates**`, `**locations/canonical**` (lists `**GET /v1/canonical-locations**`; each row’s `**id**` is a **canonical UUID string**; the API may include a catalog `**slug`** but the Stylebook UI does not surface it), `/locations/canonical/:id` (`**:id**` is that UUID; detail uses `**GET /v1/canonical-locations/{id}**`, `**…/mentions**`, and `**GET …/linked-substrates**`), stylebook-scoped `**GET|POST /v1/stylebooks/{stylebook_slug}/canonical-locations/{id}/meta**` plus `**PATCH|DELETE …/meta/{meta_id}**` for catalog JSON metadata, stylebook-scoped `**GET|POST /v1/stylebooks/{stylebook_slug}/canonical-locations/{id}/connections**` plus `**PATCH|DELETE …/connections/{id}**` for the directed connections graph (and `**GET /v1/connections/stylebooks/{stylebook_slug}/natures**` for nature typeahead), `/locations/create` (creates a catalog canonical only via `**POST /v1/canonical-locations**` or legacy `**POST /v1/locations**` — **no** `substrate_location` row; navigate using the returned canonical `**id`**), `**/people/canonical**` and `/people/canonical/:id` (detail uses `**GET /v1/stylebooks/{slug}/canonical-people/{id}**`, `**…/mentions**`, `**…/linked-substrates**`, and stylebook-scoped meta routes — same **Mentions** + **Metadata** + **Connections** layout as location; see [`ENTITY_TYPES.md`](ENTITY_TYPES.md) → **Stylebook canonical detail page**), `**/people/create**` (manual catalog person via `**POST /v1/stylebooks/{slug}/canonical-people**` — no substrate row), `**/import/people**` (CSV wizard: analyze then import with column mappings; review step allows removing rows), `**/import/locations**` (GeoJSON FeatureCollection wizard: `**POST /v1/import/geojson/analyze**` then `**POST /v1/import/geojson**` with `mappings` plus optional top-level `**meta_property_mappings**` — an optional **Metadata** step maps `properties` keys to `StylebookLocationMeta.meta_type` per feature, persisting `**data`** as `**{ "<property_key>": <value> }**` so the metadata card can show Key / Value; skip sends `[]`), plus **stub** pages for other entity candidates, and agents so deep links do not 404. The **People** list header includes **Create** and **Import** alongside **Candidates** (mirrors locations). Every canonical detail page includes **Metadata** (single card with **Add Meta** in the card header and per-entry **Edit** / **Delete** on each row) and **Connections** (single card with **Add connection** in the card header, then list + **reactflow** graph tab); organization / work canonical pickers use **empty-list stub** APIs until those entities are migrated. Metadata **data** defaults to a **key / value table** when the stored JSON is a flat object with scalar values (strings, numbers, booleans, null); otherwise the UI shows formatted JSON. **Table** and **JSON** toggles let editors switch modes; the API still stores arbitrary JSON-serializable payloads.
...
- **Client layout:** typed calls are split under `apps/stylebook-ui/src/lib/stylebook-api/` and re-exported from `src/lib/api.ts` for pages that still follow the legacy flat import style.
...
Both UIs ship same-origin bundles for CloudFront/S3 deployment: API calls use relative paths (`/api/agate`, `/api/stylebook`, `/v1/...` for auth) so one build serves every client behind path routing.

| App | Build command | Output | Env defaults |
|-----|---------------|--------|--------------|
| Agate UI | `make agate-ui-build` | `apps/agate-ui/dist/` | [`apps/agate-ui/.env.production`](../apps/agate-ui/.env.production) |
...
- **Project workspace — credentials:** (1) **API access keys** (`bfk_…`) are issued by Core API (`/v1/projects/{id}/api-keys`); the **API** tab uses `**core-api.ts`** helpers and `[ProjectAccessKeysPanel](../apps/agate-ui/src/components/ProjectAccessKeysPanel.tsx)` for Bearer access to Backfield APIs. The create-key dialog offers **Allowed access** checkboxes (**Read project data** always on; **Trigger flows via API** for org-admin **service** keys only). Listed keys show scope badges; rotate preserves scopes. (2) **Geocoding, search, and S3 overrides** for a project use Agate API `/projects/{id}/secrets` on the project **Integrations** tab (`**api.ts`**). Optional **LLM / Azure** project env keys still use the same Agate secrets API from the full **Project settings** dialog when present; prefer the **Models** tab and organization AI settings for LLM credentials.

...
- **Processed item detail tabs:** the run item page (`**/runs/{runId}/items/{itemId}**`) keeps the active tab in the URL as `**?tab=<id>**` (`info`, `places`, `people`, `organizations`, `images`, `meta`, `json`). Invalid or missing values fall back to `**info**`. A legacy `**#<tab>**` fragment on first load is promoted to `**?tab=**` for sharing.

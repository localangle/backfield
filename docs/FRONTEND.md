...
- **Stylebook home tabs:** the catalog dashboard (`**/stylebook/<slug>/**`) and cleanup routes share a tab strip (**Entities** | **Cleanup**) via [`StylebookHomeTabs`](../apps/stylebook-ui/src/components/StylebookHomeTabs.tsx). **Cleanup** is stylebook-scoped data-quality triage only—no automatic fixes. v1 ships two location checks: **Possible duplicate locations** (`**/cleanup/duplicate-locations**`, cluster view backed by `**GET /v1/stylebooks/{slug}/cleanup/checks/duplicate-locations**`) and **Locations missing geography** (`**/cleanup/missing-geometry-locations**`, flat list via `**…/cleanup/checks/missing-geometry-locations**`). The hub (`**/cleanup**`) lists checks with counts from `**GET …/cleanup/checks**`. Each flagged row deep-links into the existing location canonical detail editor.

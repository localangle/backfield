...
- `apps/worker`
  ...
  - May execute worker-local nodes (e.g. `DBOutput`) that write directly to Postgres using `backfield-db` helpers (see `apps/worker/src/worker/nodes/db_output.py`). Substrate ingest lives under `apps/worker/src/worker/substrate/` (`content/` for article, `entities/location/` for places, `canonical/` for adjudication, `orchestration.py` for `persist_from_consolidated`). After persistence, DBOutput may run post-steps that do not fail the node: **semantic indexing** (`backfield_entities.ingest.semantic_indexing`) and **automatic Stylebook connections** (`backfield_entities.connections`, gated by `auto_connections_enabled` plus AI-assisted canonicalization settings; LLM classification by endpoint family; high-confidence edges only). Adding canonical types: see [`ENTITY_TYPES.md`](ENTITY_TYPES.md) and `.cursor/skills/add-entity-type/SKILL.md`.
...
**Worker GeocodeAgent** (database cache tier) does **not** use this fallback chain for cache lookups; when cache is on, the catalog id on the node is authoritative — see **`packages/backfield-entities/src/backfield_entities/catalog/resolve.py`** module docstring for rationale.
...
- **Graph parallelism (per processed item):** When `BACKFIELD_PARALLEL_GRAPH_LEVELS` is enabled on the worker, `agate_runtime.executor.execute_graph` groups nodes by graph depth and runs all nodes at the same level concurrently via `asyncio.gather` (agate-ai-platform parity). Levels remain sequential (e.g. S3 Input, then Organization + Person + Place Extract in parallel, then Geocode, then DBOutput). Public output keys still follow topological order, not parallel completion order. LLM attempt rows use a lock around shared-session writes during parallel levels.

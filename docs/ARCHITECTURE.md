...
- `worker` may depend on `agate-runtime`, `backfield-db`, and `backfield-stylebook` (canonical sync next to substrate persistence).
- `core-api` may depend on `backfield-auth`, `backfield-db`, and `backfield-stylebook` (default Stylebook + workspace creation defaults).
- `agate-runtime` may depend on `backfield-db` and `backfield-stylebook` for shared executor/runtime wiring (for example the Geocode cache bundle), but must not depend on app code.

...
- `apps/worker`
  ...
  - **S3Input batch runs:** `agate-api` enqueues **`execute_s3_batch_setup`** when the stored graph contains an **S3Input** node. That task lists `*.json` keys under the node’s bucket/prefix (using decrypted project AWS keys), validates each document’s top-level **`text`**, writes **`agate_processed_item`** rows, then dispatches **`execute_processed_item`** in chunks bounded by **`S3_BATCH_MAX_INFLIGHT`**. Each child task runs the same `execute_graph` as a normal run but replaces the **S3Input** runner with a shim fed from the row’s **`input_json`**; **`BACKFIELD_RUN_ID`** stays the parent **`agate_run.id`** so DBOutput substrate writes stay tied to one batch run.

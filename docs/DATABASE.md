...
Schema revisions start at `001_agate_baseline` (initial `agate_*` tables and seed rows). Revision **`002_starter_flow_layout`** is a **data migration** that rewrites stored `spec_json` for graphs named `Starter flow` (and the seeded **Geocode pipeline** template) so node positions match the UI card widths—run `make migrate` so existing databases pick up the layout fix.

...
Revision **`052_location_canonical_label_trgm`** (Postgres only) adds GIN trigram index **`ix_stylebook_location_canonical_label_trgm`** on **`lower(label)`** for Stylebook cleanup duplicate-location detection (requires **`pg_trgm`** from **`014`**). Non-Postgres upgrades skip the index.

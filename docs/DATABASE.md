...
- `substrate_article_meta` — one row per article and `meta_type` (Article Metadata node → DBOutput persist): `category`, `rationale`, `confidence` (0–1), optional `prompt_preset`, optional `source_run_id` FK to `agate_run`, timestamps. Unique on `(article_id, meta_type)`.

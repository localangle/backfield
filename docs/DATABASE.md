...
Revision **`058_stylebook_candidate_ai_review`** adds **`stylebook_candidate_ai_review`** (background AI review runs for candidate queues: `entity_type` person|organization|location, `project_id`, model selection, `candidate_count` / `processed_count` / `recommendation_count`, status). The worker task **`execute_candidate_ai_review`** re-runs decide + LLM adjudication per open queue row and writes **`canonical_suggestion`** (+ **`canonical_adjudication`** when applicable) onto each substrate row's **`canonical_review_reasons_json`** without linking. Acceptance uses the existing candidate accept/link/defer endpoints and the queue **Accept all AI recommendations** action.

...
- `substrate_article_meta` — article metadata tags (Article Metadata node → DBOutput persist): `category`, `rationale`, `confidence` (0–1), optional `prompt_preset`, optional `source_run_id` FK to `agate_run`, timestamps. Unique on `(article_id, meta_type, category)` so multi-value presets such as **Topic** can persist up to three rows per article. Composite index **`ix_substrate_article_meta_type_category`** `(meta_type, category)` supports public article and mention search filters on section/topic/subject/format metadata (revision **`049_public_read_perf_indexes`**).
...
Revision **`049_public_read_perf_indexes`** adds composite **`ix_substrate_article_meta_type_category`** on **`substrate_article_meta (meta_type, category)`** for public article and mention metadata filters. On Postgres only, adds expression GiST indexes **`idx_substrate_location_geometry_geography_gist`** and **`idx_stylebook_location_canonical_geometry_geography_gist`** on **`(geometry::geography)`** so public geo search queries that cast geometry to geography can use an index (distinct from the existing geometry-opclass GiST indexes).

Revision **`050_agate_item_timing`** adds nullable **`started_at`** on **`agate_processed_item`** and creates **`agate_node_timing`** (per-node wall-clock rows for batch items) with indexes on **`run_id`**, **`processed_item_id`**, and **`node_type`**.
...
Revision **`052_sb_loc_label_trgm`** (Postgres only) adds GIN trigram index **`ix_stylebook_location_canonical_label_trgm`** on **`lower(label)`** (requires **`pg_trgm`** from **`014`**). Non-Postgres upgrades skip the index.
...
Revision **`062_stylebook_cleanup_check_run`** adds **`stylebook_cleanup_check_run`** (queued/running/succeeded/failed cleanup check executions keyed by stylebook, check id, and scope hash) and **`stylebook_cleanup_check_result`** (generic cached candidate rows for a run: cluster or list item, canonical ids, dismissal pair keys, payload JSON, searchable text, ordinal). Indexes: **`(stylebook_id, check_id, scope_hash, status, completed_at)`** and **`(stylebook_id, check_id, scope_hash, created_at)`** on runs; **`(run_id, ordinal)`** and unique **`(run_id, item_key)`** on results. The worker task **`execute_cleanup_check_run`** builds finder output once per run; Stylebook API hub/detail routes read persisted rows and apply dismissals at read time.

Revision **`058_sb_candidate_ai_review`** adds **`stylebook_candidate_ai_review`** (background AI review runs for candidate queues: `entity_type` person|organization|location, `project_id`, model selection, `candidate_count` / `processed_count` / `recommendation_count`, status). The worker task **`execute_candidate_ai_review`** re-runs decide + LLM adjudication per open queue row and writes **`canonical_suggestion`** (+ **`canonical_adjudication`** when applicable) onto each substrate row's **`canonical_review_reasons_json`** without linking. Acceptance uses the existing candidate accept/link/defer endpoints and the queue **Accept all AI recommendations** action.
...
The **General** project and default workspace are ensured at runtime when `BACKFIELD_LOCAL_BOOTSTRAP=1` on `agate-api` startup (see [docs/OPERATIONS.md](OPERATIONS.md)). Agate graphs are not seeded at bootstrap.

...
- **Persisted story places:** When a merged row includes **`persisted_location_id`**, map geometry saves go to **`PATCH /v1/locations/{location_id}/geometry?project_slug=…`** (Stylebook API) and update the saved place for this story. The review payload may reflect saved geometry on the merged row after refresh. **`DELETE /v1/locations/{location_id}?project_slug=…`** soft-deletes active mentions; optional **`article_id`** limits that to one article. The substrate row is deleted when no active mentions remain.
...
- **`merged_locations`**: array of `{ "anchor", "source": "model"|"user", "node_id", "index_in_node", "stale", "location", … }`. **`stale`** is always **`false`** for rows included here; the lane lists current model rows (with patches applied) plus valid **`user_place:*`** rows. When a row matches a persisted substrate location for the item’s article and run, the API may add:

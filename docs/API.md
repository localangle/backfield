...
- **Org admin** (session `org_role` = `org_admin` for the same `organization_id` as the path):
  ...
  - **Organization integration secrets** (`backfield_organization_integration_secret`): org admins manage encrypted vendor credentials. Responses never include plaintext or ciphertext — only metadata (`integration_key`, timestamps). Writes require **`MASTER_ENCRYPTION_KEY`** (same Fernet path as project secrets).
    - `GET /v1/organizations/{org_id}/integration-secrets/ai-provider-catalog` — OpenAI, Anthropic, Gemini, OpenRouter, Azure OpenAI (API key + endpoint URL slots), with `configured` plus timestamps when present.
    - `GET /v1/organizations/{org_id}/integration-secrets` — stored rows for the org (metadata only).
    - `PUT /v1/organizations/{org_id}/integration-secrets/{integration_key}` — JSON body `{ "value": "<secret>" }`; replaces or creates. Allowed keys in v1 include **`ai.provider.openai`**, **`ai.provider.anthropic`**, **`ai.provider.gemini`** (**`GEMINI_API_KEY`**), **`ai.provider.openrouter`** (**`OPENROUTER_API_KEY`**), **`ai.provider.azure`** (**`AZURE_API_KEY`**), and **`ai.provider.azure.api_base`** (resource endpoint URL string → **`AZURE_API_BASE`** for LiteLLM).

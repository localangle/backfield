...
- `routers/runs.py`
  - Create, list, fetch, and cancel runs; enqueue **`worker.tasks.execute_s3_batch_setup`** when the graph spec includes an **S3Input** node, otherwise **`worker.tasks.execute_agate_run`**. **`POST /runs/{id}/cancel`** stops a **`pending`** or **`running`** run (marks the run **`failed`** with a fixed cancellation message, fails in-flight batch **`agate_processed_item`** rows). **`GET /runs/{id}`** includes **`processed_items`** (``agate_processed_item`` rows), each with **`estimated_ai_cost`** / **`estimated_ai_cost_incomplete`** / **`estimated_ai_cost_currency`** rolled up from **`backfield_ai_call_record`**, plus **`whole_run_ai_cost_*`** for LLM rows not tied to a batch item (single-graph runs). **`GET /runs/{id}/items/{item_id}`** returns one item’s parsed **`input`** / **`output`** (child graph **`result_json`**) and the same cost rollup fields.
- `routers/nodes.py`

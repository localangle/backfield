...
- **Canonical people (Stylebook UI):** `**GET /v1/canonical-people?project_slug=…**` lists `**stylebook_person_canonical**` rows for the project Stylebook with optional `**q**`, `**type_filter**`, `**public_figure**`, and `**nature_filter**` (canonicals with at least one linked mention of that nature). Detail, create, patch, link/unlink substrate, and linked-substrate list routes live under `**/v1/canonical-people/{canonical_id}…**` with `**project_slug**`. Canonical ids are **UUID strings**.
...
- **Canonical person metadata:** `**GET` / `POST` / `PATCH` / `DELETE` `/v1/canonical-people/{canonical_id}/meta…**` with `**project_slug**` (parity with location meta).

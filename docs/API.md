...
6. Client polls `GET /runs/{id}` until the run reaches a terminal state (refresh `**processed_items**` for per-file batch progress). `**POST /runs/{id}/items/{item_id}/rerun**` re-queues a single batch file or a whole-graph run (`**items/1**` when there are no batch rows) without starting a new run; it also clears review overlay. Saved places are reconciled only when Stylebook Output runs.
...
## Stylebook Output reconciliation (places v1)

Stylebook Output owns saved Stylebook/substrate writes for domains produced by the flow. In this slice, the only implemented owned domain is **Places**. Article identity is assumed to be supplied upstream; reconciliation does not infer identity from fuzzy text or URL matches.
...
- **`locations.by_anchor[anchor].occurrences`**: optional array of story mention evidence for that place. Each element: `{ "client_id"?: string (UUID for new rows), "id"?: number (substrate occurrence id when known), "mention_text": string, "start_char"?: number|null, "end_char"?: number|null, "occurrence_order"?: number, "suppressed"?: boolean }`. Patches replace/merge by **`id`** or **`client_id`**. Max **50** per place. **`mention_text`** is required when not suppressed.
...
- **`merged_locations`**: array of `{ "anchor", "source": "model"|"user", "node_id", "index_in_node", "stale", "location", … }`. **`stale`** is always **`false`** for rows included here; the lane lists current model rows (with patches applied) plus valid **`user_place:*`** rows, including Geocode **`needs_review`** rows. Rows with **`geocode_qa_code`: `geocode_region_mismatch`** (resolver country/centroid disagrees with PlaceExtract **`components.country`**) omit map geometry so verification does not plot wrong pins. When a row matches a persisted substrate location for the item’s article and run, the API may add:

...
- **Canonical people (Stylebook UI):** `**GET /v1/canonical-people?project_slug=…**` lists `**stylebook_person_canonical**` rows for the project Stylebook with optional `**q**`, `**type_filter**`, `**public_figure**`, and `**nature_filter**` (canonicals with at least one linked mention of that nature). Detail, create, patch, link/unlink substrate, linked-substrate list, and mentions list routes live under `**/v1/canonical-people/{canonical_id}…**` or stylebook-scoped `**/v1/stylebooks/{stylebook_slug}/canonical-people/{canonical_id}…**` with optional `**project**` / `**project_slug**`. Canonical ids are **UUID strings**. Stylebook-scoped mentions: `**GET /v1/stylebooks/{stylebook_slug}/canonical-people/{canonical_id}/mentions**` (paginated; optional `**project**` filter; rows include `**substrate_person_id**` for grouping with `**GET …/linked-substrates**`).
- **Person candidate queue:** flat paginated list at `**GET /v1/people/candidates?project_slug=…&status=open|deferred**` (no clustering). Actions: `**POST …/candidates/{substrate_person_id}/accept**` (`**create_new**` or `**stylebook_person_canonical_id**`), `**POST …/defer**`, `**POST …/note**`, `**GET …/context**`, `**GET …/suggested-canonicals**`. `**GET /v1/people/candidates/types**` returns distinct pending person types.
...
**Future entity types** (organization, work): per-type routers under `stylebook_api/entities/<type>/` follow the same pattern as person. Shared helpers live in `stylebook_api/helpers/project_scope.py` and `pagination.py`. Entity slugs and consolidated JSON keys are defined in `backfield_stylebook.entity_types`.

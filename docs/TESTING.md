...
3. **Smoke suite**
  ...
  - `make smoke`: the Agate-to-Stylebook handoff lane (`[tests/smoke/golden_path_stack.py](../tests/smoke/golden_path_stack.py)`). In session mode it follows **log in → workspace → project → run**; otherwise it uses the service Bearer path. It creates the **Starter flow** graph via the Agate API when missing, asserts the run `result` includes **`stylebook_output`** with `success: true` (via `GET /runs/{id}/items/{item_id}` when the run uses processed items), and checks the persisted article/location is visible through downstream Stylebook read APIs.
  ...
  - `make smoke-article-metadata`: optional in-process ArticleMetadata + DBOutput smoke with mocked LLM (not part of CI).
  - `make smoke-article-metadata-stack`: optional stack harness for the **Article Metadata starter** graph (`starter_article_metadata_flow_graph_spec`); not created by local bootstrap—create the graph or set `SMOKE_ARTICLE_METADATA_GRAPH_ID`.
  - `make smoke-custom-extract`: optional in-process CustomExtract + DBOutput smoke with mocked LLM (recipe article → ingredients records; asserts schema, grounded records, and the `dropped_ungrounded` count; not part of CI).
  - `make smoke-custom-extract-stack`: optional stack harness for the **Custom Extract starter** graph (`starter_custom_extract_flow_graph_spec`); not created by local bootstrap—create the graph or set `SMOKE_CUSTOM_EXTRACT_GRAPH_ID`. Also asserts `substrate_custom_record` rows exist for the persisted article.

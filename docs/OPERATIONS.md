...
- **Required for LLM PlaceExtract**: `OPENAI_API_KEY` and/or `ANTHROPIC_API_KEY` (see `agate_utils.llm.call_llm`). PlaceExtract writes **`geocode_hints`** on each location when extra story context helps geocoding; GeocodeAgent forwards them into **`place`** web-search query generation and best-address extraction prompts.

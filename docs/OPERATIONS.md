...
- `BACKFIELD_PARALLEL_GRAPH_LEVELS`: when **`1`**, **`true`**, or **`yes`**, the worker runs independent graph branches at the same depth in parallel inside each **`execute_processed_item`** / **`execute_agate_run`** (e.g. Organization + Person + Place Extract after a shared input). Local Compose defaults to **`1`** on the **`worker`** service; set **`0`** in repo-root `.env` to disable. See [docs/ARCHITECTURE.md](ARCHITECTURE.md). Distinct from **`CELERY_WORKER_CONCURRENCY`**, which parallelizes *different* batch files.

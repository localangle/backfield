...
- Worker task names:
  ...
  - `worker.tasks.export_stylebook_bundle` — builds a stylebook ZIP (**manifest + canonical JSONL shards**, plus **aliases**, **project-scoped meta**, and **connections** in schema **4**; no substrate, candidate-queue, cache, or activity-log data) and uploads it to **`STYLEBOOK_BUNDLE_S3_BUCKET`** for org-admin download links from **stylebook-api**. Schema **4** exports location, person, and organization canonical rows and editorial sidecars. Older bundles (schema **1**–**3**) contain canonical rows only.
  - `worker.tasks.import_stylebook_bundle` — downloads a staged ZIP from the same bucket and imports into a **new** stylebook (new canonical UUIDs). Manifest schema **1**–**4** is accepted; optional `project_mappings` remaps project slugs for meta and connections. Legacy `kind: canonical` shards are treated as locations.
...
- `BACKFIELD_SQLALCHEMY_POOL_SIZE` / `BACKFIELD_SQLALCHEMY_MAX_OVERFLOW`: optional SQLAlchemy pool sizing for `backfield_db.session.get_engine()` (defaults follow SQLAlchemy when unset: **5** / **10**). API services use local Compose defaults **3** / **2**. The local **`worker`** service maps worker-only knobs **`WORKER_BACKFIELD_SQLALCHEMY_POOL_SIZE`** / **`WORKER_BACKFIELD_SQLALCHEMY_MAX_OVERFLOW`** into those runtime env vars, defaulting to **1** / **0** so Celery children cannot consume the full PgBouncer pool during large batches. Raise worker overflow to **1** only if worker logs show local SQLAlchemy pool checkout timeouts.
...
- `CANONICAL_ADJUDICATION_MAX_CONCURRENT`: max parallel LLM calls per Backfield Output domain pass during **`ai_assisted`** canonical adjudication (places, people, organizations), and for **questionable organization canonicals** cleanup check batch review (session-free batched calls in the worker). Default **8**; set **`1`** for serial LLM behavior. DB upserts, plan application, and mention writes stay serial on one SQLModel session. Tune alongside **`CELERY_WORKER_CONCURRENCY`** if OpenAI rate limits appear.
- `DBOUTPUT_MAX_CONCURRENT_PERSISTS`: max Backfield Output / **DBOutput** persistence sections allowed across worker child processes at once (default **8**, set **0** to disable). The gate uses Redis locks, so waiting workers do not hold Postgres connections. Lower this before lowering **`CELERY_WORKER_CONCURRENCY`** when API latency is caused by write bursts; extraction and geocoding can remain parallel while the database-heavy persist phase backs off.
- `DBOUTPUT_PERSIST_LOCK_TTL_S`: Redis lock TTL for the DBOutput persist gate (default **1800** seconds). Increase only if a single DBOutput persist can legitimately run longer than the TTL.
- `CANDIDATE_AI_REVIEW_MAX_CONCURRENT`: max parallel LLM workers for Stylebook **candidate queue AI review** (worker task). Defaults to **`min(CANONICAL_ADJUDICATION_MAX_CONCURRENT, 3)`** so post-LLM writes and progress updates stay within the worker SQLAlchemy pool (Compose default pool **1** + overflow **0**). Candidate review releases DB connections before each LLM call; raise this if you also raise worker pool size.
...
- **Celery limits**: `TASK_SOFT_TIME_LIMIT` / `TASK_HARD_TIME_LIMIT` (Compose defaults **`900`** / **`1080`** seconds — 15 / 18 minutes — on the worker) are enforced on **`worker.tasks.execute_processed_item`** so a hung per-article task fails instead of stalling an S3 batch tail. The default preserves PlaceExtract’s configured **`llmTimeout`** (600s) alongside the nodes’ internal 300s Celery buffer; do not lower the soft limit to 600s without also revisiting that buffer. **`execute_processed_item`** uses **`acks_late`** and **`task_reject_on_worker_lost`** so broker redelivery can resume work after a worker crash; rows left in **`running`** longer than **`TASK_HARD_TIME_LIMIT` + `TASK_STALE_RUNNING_GRACE_S`** (default grace **300** s) are treated as stale — **`finalize_s3_parent_run`** and each **`execute_processed_item`** start call **`_reap_stale_running_items_for_run`** so zombie **`running`** rows are cleared during long batches, not only at finalize. When a batch run shows far more **Running** items in the Agate UI than **`celery inspect active`** (Compose default concurrency **16**), orphan **`running`** rows are usually leftover claims after worker-child crashes; the worker periodically releases them back to **`pending`** when they are not actively executing (**`BATCH_ORPHAN_RUNNING_AFTER_S`**, default **120** s) and throttled by **`BATCH_ORPHAN_RECONCILE_INTERVAL_S`** (default **30** s). Re-run failed items from the Agate UI when needed.
...
- If Postgres logs **`FATAL: sorry, too many clients already`**, check PgBouncer **`SHOW POOLS`** / **`SHOW STATS`** first (local Compose). Every service process that imports `backfield_db.session` holds one pooled engine: reduce **`CELERY_WORKER_CONCURRENCY`**, lower **`WORKER_BACKFIELD_SQLALCHEMY_POOL_SIZE`** / **`WORKER_BACKFIELD_SQLALCHEMY_MAX_OVERFLOW`**, tune **`DBOUTPUT_MAX_CONCURRENT_PERSISTS`**, tune PgBouncer **`MAX_DB_CONNECTIONS`**, or raise Postgres **`max_connections`** in your deployment config. Avoid ad-hoc `create_engine` in long-lived workers — use **`get_engine()`** so each process has a single pool.
...
- If **`EmbedText`** or other LiteLLM nodes fail with **`deadlock detected by _ModuleLock('openai.resources.chat')`**, parallel graph branches are racing the OpenAI SDK’s lazy imports. Each Celery child process warms LiteLLM/OpenAI imports on startup via **`worker_process_init`**; rebuild/restart the worker after upgrading. As a mitigation, set **`BACKFIELD_PARALLEL_GRAPH_LEVELS=0`** in repo-root `.env` to run graph nodes sequentially.
- If the worker container is **OOM-killed** (children die with `signal 9 (SIGKILL)` and no Celery time-limit log), total memory is roughly **`CELERY_WORKER_CONCURRENCY` × (import baseline ≈ 320 MB + per-article working set)**. LiteLLM alone accounts for ~155 MB of the baseline. To limit per-child duplication, the worker parent warms shared imports and calls **`gc.collect()` + `gc.freeze()`** on **`worker_init`** before prefork children spawn, and each child freezes again after warmup — frozen objects stay in copy-on-write pages shared with the parent instead of being duplicated per child. If OOM kills persist, lower **`CELERY_WORKER_CONCURRENCY`** in repo-root `.env` or raise the Docker VM memory allocation.

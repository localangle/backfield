...
- If **`uv run backfield`** fails with **`ModuleNotFoundError: No module named 'backfield_cli'`**, the repo-root `.venv` has a stale editable install (often after a partial `uv sync` from an app package directory, or on macOS when the clone lives under Desktop/iCloud). From the repo root run **`make bootstrap`**, then retry. `backfield init` also repairs tooling at start and finish; avoid running bare `uv sync` from `apps/*` or `packages/*` — use **`uv sync --all-packages`** at the repo root instead.

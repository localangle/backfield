...
**`agate-api` and `worker` images** copy `packages/backfield-ai` and install editable wheels in dependency order (`backfield-db` → `backfield-ai` → `backfield-agate` → …) because those package names are not published on PyPI.

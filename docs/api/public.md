...
Project keys include the `read` scope when minted. Starting a run additionally
requires `runs:trigger`; that scope is limited to service-type project keys.
...
- Public paths use plural product nouns and remain nested beneath a project
  slug.
- Routers parse and validate HTTP input, enforce project access, and delegate
  query work to shared packages such as
  [`backfield-entities`](../../packages/backfield-entities/).
...
Core API uses FastAPI's generated schema at `/openapi.json` and interactive
documentation at `/docs`. The schema contains Core API's other routes as well;
public operations are identifiable by their `/public/v1` paths and
`public-*` tags. Treat the generated schema and the route response models as

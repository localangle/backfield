...
from agate_nodes.geocode_agent.nodes.route_strategy import (
    fallback_strategy_for_location_type,
    route_strategy_node,
)


def test_fallback_strategy_place_vs_city() -> None:
    assert fallback_strategy_for_location_type("place") == "web_search"
    assert fallback_strategy_for_location_type("city") == "no_web_search"
    assert fallback_strategy_for_location_type("street_road") == "no_web_search"
    assert fallback_strategy_for_location_type("region_state") == "no_web_search"
...
def test_route_strategy_fallback_without_openai_key() -> None:
    async def _run() -> None:
        ...
        await route_strategy_node(state)
        assert state.get("geocode_strategy") == "web_search"
...
def test_route_strategy_llm_selects_web_search() -> None:
    async def _run() -> None:
        def _fake_llm(*_a: object, **_k: object) -> str:
            return json.dumps({"strategy": "web_search", "rationale": "need address"})

        state: dict = {
            "location_type": "place",
            "location_text": "Spyhouse",
            "location_components": {},
            "original_text": "",
            "openai_api_key": "sk-test",
            "router_llm_model": "gpt-4o-mini",
...
def test_route_strategy_fallback_place_after_invalid_llm_json() -> None:
    async def _run() -> None:
        def _bad_llm(*_a: object, **_k: object) -> str:
            return "not-json"

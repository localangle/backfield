...
def test_delete_article_meta_overlay_only_json_output(tmp_path, monkeypatch) -> None:
    database_path = tmp_path / "agate-meta-delete-overlay.db"
    engine = create_engine(
        f"sqlite:///{database_path}",
        connect_args={"check_same_thread": False},
    )
    ...
    try:
        ...
        graph = tc.post(
            ...
            json={
                "name": "Batch",
                "project_id": project["id"],
                "spec": _minimal_text_input_spec(name="meta-delete"),
            },
        ).json()
        with Session(engine) as session:
            run_row = _insert_pending_run(session, graph["id"])
        ...
        assert response.status_code == 200
        payload = response.json()
        assert payload["article_meta"] == []
        reviewed = payload["reviewed_output"]
        assert "article_metadata" not in reviewed["json_output"]["consolidated"]
    finally:
        app.dependency_overrides.clear()


def test_delete_article_meta_removes_substrate_row(tmp_path, monkeypatch) -> None:
    database_path = tmp_path / "agate-meta-delete-substrate.db"
    engine = create_engine(
        f"sqlite:///{database_path}",
        connect_args={"check_same_thread": False},
    )
    ...
    try:
        ...
        graph = tc.post(
            ...
            json={
                "name": "Batch",
                "project_id": project["id"],
                "spec": _minimal_text_input_spec(name="meta-delete-substrate"),
            },
        ).json()
        with Session(engine) as session:
            article_id, meta_id = _seed_article_with_meta(
...
def test_create_article_meta_overlay_only_json_output(tmp_path, monkeypatch) -> None:
    database_path = tmp_path / "agate-meta-create-overlay.db"
    engine = create_engine(
        f"sqlite:///{database_path}",
        connect_args={"check_same_thread": False},
    )
    ...
    try:
        ...
        graph = tc.post(
            ...
            json={
                "name": "Batch",
                "project_id": project["id"],
                "spec": _minimal_text_input_spec(name="meta-create"),
            },
        ).json()
        with Session(engine) as session:
            run_row = _insert_pending_run(session, graph["id"])
            ...
            item = AgateProcessedItem(
                ...
                input_json='{"headline":"Story","text":"Body"}',
                status="succeeded",
                result_json='{"json_output":{"consolidated":{"headline":"Story","text":"Body"}}}',
            )
            session.add(item)
            session.commit()
            session.refresh(item)
        ...
        response = tc.post(
            f"/runs/{rid}/items/{iid}/article-meta",
            json={"meta_type": "subject", "category": "Local news"},
            headers={"If-Match": '"0"'},
        )
        assert response.status_code == 200
        payload = response.json()
        ...
        assert payload["article_meta"][0]["category"] == "Local news"
        assert payload["article_meta"][0]["source"] == "review"
        assert payload["article_meta"][0]["id"] < 0
        reviewed = payload["reviewed_output"]
        assert (
            reviewed["json_output"]["consolidated"]["article_metadata"]["category"]
            == "Local news"
        )
    finally:
        app.dependency_overrides.clear()
...
def test_create_article_meta_persists_substrate_row(tmp_path, monkeypatch) -> None:
    database_path = tmp_path / "agate-meta-create-substrate.db"
    engine = create_engine(
        f"sqlite:///{database_path}",
        connect_args={"check_same_thread": False},
    )
    ...
    try:
        ...
        graph = tc.post(
            ...
            json={
                "name": "Batch",
                "project_id": project["id"],
                "spec": _minimal_text_input_spec(name="meta-create-substrate"),
            },
        ).json()
        with Session(engine) as session:
            article = SubstrateArticle(
        ...
        response = tc.post(
            f"/runs/{rid}/items/{iid}/article-meta",
            json={"meta_type": "format", "category": "News story"},
            headers={"If-Match": '"0"'},
        )
        assert response.status_code == 200
        payload = response.json()
        assert len(payload["article_meta"]) == 1
        assert payload["article_meta"][0]["meta_type"] == "format"
        meta_id = payload["article_meta"][0]["id"]
        assert meta_id > 0

        with Session(engine) as session:
            row = session.get(SubstrateArticleMeta, meta_id)
            assert row is not None
            assert row.category == "News story"
    finally:
        app.dependency_overrides.clear()

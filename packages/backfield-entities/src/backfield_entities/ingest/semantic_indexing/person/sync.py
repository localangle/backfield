"""Synchronize person semantic document rows with builder output."""

from __future__ import annotations

from backfield_db import SubstratePersonSemanticDocument
...
from backfield_entities.ingest.semantic_indexing.person.builder import build_occurrence_document
from backfield_entities.ingest.semantic_indexing.person.loader import load_sync_bundles
...
def sync_semantic_documents(
    ...
    *,
    ...
    existing_by_occurrence = {
        int(row.person_mention_occurrence_id): row for row in existing_rows
    }
    ...
    def create_row(draft: SemanticDocumentDraft) -> None:
        session.add(
            SubstratePersonSemanticDocument(
                ...
                person_id=draft.entity_id,
                person_mention_id=draft.mention_id,
                person_mention_occurrence_id=draft.occurrence_id,
    ...
    def update_row(row: SubstratePersonSemanticDocument, draft: SemanticDocumentDraft) -> None:
        row.person_id = draft.entity_id
        row.person_mention_id = draft.mention_id
    ...
    def deactivate_row(row: SubstratePersonSemanticDocument) -> None:
        row.active = False
        row.stale = True
        session.add(row)

    def reactivate_row(row: SubstratePersonSemanticDocument) -> None:
        row.active = True
        row.stale = False
        session.add(row)

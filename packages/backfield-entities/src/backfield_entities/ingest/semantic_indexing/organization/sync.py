"""Synchronize organization semantic document rows with builder output."""

from __future__ import annotations

from backfield_db import SubstrateOrganizationSemanticDocument
from backfield_db.semantic_indexing import SEMANTIC_EMBEDDING_STATUS_PENDING
from sqlmodel import Session, select

from backfield_entities.ingest.semantic_indexing.common.article import load_article_source
...
from backfield_entities.ingest.semantic_indexing.organization.loader import load_sync_bundles
from backfield_entities.ingest.semantic_indexing.sync_apply import apply_semantic_document_sync
from backfield_entities.ingest.semantic_indexing.sync_contract import SemanticSyncSummary
...
def sync_semantic_documents(
    ...
    *,
    ...
    if article is None:
        return SemanticSyncSummary(entity_type="organization")

    bundles = load_sync_bundles(session, article_id=article_id)
    expected: dict[int, SemanticDocumentDraft] = {}
    inactive: set[int] = set()

    for organization, mention, occurrence, canonical in bundles:
    ...
    existing_by_occurrence = {
        int(row.organization_mention_occurrence_id): row for row in existing_rows
    }
    ...
    def create_row(draft: SemanticDocumentDraft) -> None:
        session.add(
            SubstrateOrganizationSemanticDocument(
                ...
                organization_id=draft.entity_id,
                organization_mention_id=draft.mention_id,
                organization_mention_occurrence_id=draft.occurrence_id,
    ...
    def update_row(
        ...
        draft: SemanticDocumentDraft,
    ) -> None:
        row.organization_id = draft.entity_id
        row.organization_mention_id = draft.mention_id
    ...
    def deactivate_row(row: SubstrateOrganizationSemanticDocument) -> None:
        row.active = False
        row.stale = True
        session.add(row)

    def reactivate_row(row: SubstrateOrganizationSemanticDocument) -> None:
        row.active = True
        row.stale = False
        session.add(row)
    ...
    return apply_semantic_document_sync(
        ...
        summary_entity_type="organization",
    )

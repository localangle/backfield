...
class Place(Address):
    ...
    def __init__(self, **kwargs):
        ...
        # Street line from extract ``components.address`` (preferred over web search).
        self._explicit_street_address: Optional[str] = None
    ...
    async def geocode(
        ...
        *,
        ...
        explicit_street = (self._explicit_street_address or "").strip()
        if explicit_street:
            self._prep = lambda: self._update_prep_with_address(
                {
                    "street": explicit_street,
                    "city": self.city or "",
                    "state": self.state_abbr or "",
                    "zipcode": "",
                    "country": self.country or "USA",
                }
            )

...
/**
 * Build visualization descriptor for AdvancedGeocodeAgent node output.
  ...
  function addPolygon(
    ...
    if (geometryType === 'MultiPolygon') {
      if (!Array.isArray(coords) || coords.length === 0) {
        console.warn(`AdvancedGeocodeAgent: MultiPolygon coordinates is not a valid array`, { coords, geometryType })
        return
      }
      ...
      for (const polygon of coords) {
        if (!Array.isArray(polygon) || polygon.length === 0) {
          console.warn(`AdvancedGeocodeAgent: Invalid polygon in MultiPolygon`, { polygon })
          continue
        }
        // First ring is the exterior ring
        const exteriorRing = polygon[0]
        if (!Array.isArray(exteriorRing)) {
          console.warn(`AdvancedGeocodeAgent: Invalid exterior ring in MultiPolygon`, { exteriorRing })
          continue
        }

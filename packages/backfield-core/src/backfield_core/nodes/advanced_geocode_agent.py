"""AdvancedGeocodeAgent — LangGraph geocoding with per-node model overrides (vendored runtime)."""
...
from backfield_agate.runners import run_advanced_geocode_agent_runtime
...
def run_advanced_geocode_agent(params: dict[str, Any], inputs: dict[str, Any]) -> dict[str, Any]:
    ctx = AgateEnvContext()
    ...
    if raw_pid and raw_sid is not None and raw_sid != "":
        try:
            ...
            logger.debug(
                "AdvancedGeocode DB cache skipped: invalid BACKFIELD_PROJECT_ID "
                "or stylebookId (%r, %r)",
                raw_pid,
                raw_sid,
            )
    ...
    return run_advanced_geocode_agent_runtime(params, inputs, ctx)

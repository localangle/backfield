...
## Street-level: public named place vs private residence (`address_place_kind`)
...
- **`public_named`** — A named business, landmark, civic building, school, hospital, retail site, or other **identifiable public or semi-public** place the story maps to real-world geography (even when the string looks like an address).
- **`private_residence`** — Primarily a **home, apartment, residential lot, or anonymous residential block** (including generic “N00 block of …” when the story reads as a residence or private dwelling). Use this when the site is not editorially a named public venue.

...
/** Resolved article metadata for the item info form (overlay → output → input → article context). */
export function readArticleFieldsFromProcessedItem(item: ProcessedItem): ArticleFields {
  ...
  for (const key of ARTICLE_FIELD_KEYS) {
    ...
    out[key] =
      pickString(overlayArticle, keys) ||
      pickString(output, keys) ||
      pickString(input, keys) ||
      (key === 'headline' && ctxHeadline ? ctxHeadline : '')
  }
  return out
}

...
import { organizationNatureBadgeClass, organizationNatureDisplayLabel } from '@/lib/organizationMentionNature'
...
function OrganizationNatureBadges({ nature }: { nature: string }) {
  if (!nature.trim()) return null
  return (
    <div className="flex flex-wrap gap-1.5">
      <Badge
        variant="outline"
        className={cn('w-fit font-medium shadow-none text-[11px]', organizationNatureBadgeClass(nature))}
      >
        {organizationNatureDisplayLabel(nature)}
      </Badge>
    </div>
  )
}
...
export function OrganizationsTable({
  ...
  if (rows.length === 0) {
    return (
      <p className="text-xs text-muted-foreground">No organizations were extracted for this story.</p>
    )
  }
      ...
      <Table className="w-full table-fixed">
        <TableHeader>
          <TableRow className="hover:bg-transparent">
            ...
            <TableHead className="h-7 w-[40%] px-2 py-0 text-[11px] font-medium">Name</TableHead>
            <TableHead className="h-7 w-[24%] px-2 py-0 text-[11px] font-medium">Type</TableHead>
            <TableHead className="h-7 px-2 py-0 text-[11px] font-medium">Nature</TableHead>
        ...
        <TableBody>
          {rows.map((row, idx) => {
            ...
            const name = organizationDisplayName(row)
            const orgType = typeof organization.type === 'string' ? organization.type.trim() : ''
            const nature = typeof organization.nature === 'string' ? organization.nature : ''
            const role = typeof organization.role_in_story === 'string' ? organization.role_in_story.trim() : ''
            const canExpand = Boolean(role)
            const expanded = Boolean(anchor && expandedAnchors.has(anchor))
            const selected = selectedAnchor === anchor
            const typeLabel = orgType ? placeExtractTypeLabel(orgType) : '—'
            const natureLabel = nature ? organizationNatureDisplayLabel(nature) : '—'

            return (
              <Fragment key={anchor || `organization-${idx}`}>
                <TableRow
                  id={anchor ? `organizations-row-${anchor}` : undefined}
                  tabIndex={0}
                    ...
                    <div className="flex items-center justify-end gap-0.5">
                      ...
                      {onDeleteOrganization ? (
                        <Button
                          ...
                          aria-label="Remove organization from story"
                      ...
                      {!onOpenStylebook && !onDeleteOrganization ? (
                  ...
                  <TableRow className="hover:bg-transparent">
                    <TableCell colSpan={5} className="bg-muted/25 px-3 py-2">
                      <div className="flex flex-col gap-2">
                        <OrganizationNatureBadges nature={nature} />
                        {role ? (

...
export function preventBlurOnPointerDown(event: React.MouseEvent) {
  event.preventDefault()
}

export type InlineNameEditorHandle = {
  /** Return the in-progress draft when editing, otherwise the saved value. */
  commitPendingEdit: () => string
}

type InlineNameEditorProps = {
  value: string
  onSave: (next: string) => Promise<void>
  /** Sync draft/committed name to parent immediately (before async onSave). */
 ...
 */
export const InlineNameEditor = forwardRef<InlineNameEditorHandle, InlineNameEditorProps>(
  function InlineNameEditor(
    ...
    useImperativeHandle(
      ...
      () => ({
        commitPendingEdit: () => {
          const next = (editingNameRef.current ? nameDraftRef.current : valueRef.current).trim()
          if (next) {
            onCommit?.(next)
          }
